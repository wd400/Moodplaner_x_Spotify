...
class _CollectionGeneratorState extends State<CollectionGenerator> {
  ...
  Future<List> downloadPlaylistData() async {
    ...
    int offset=0;
    while (true) {
      var url = "https://api.spotify.com/v1/playlists/${context
          .read(currentPlaylistIdProvider)
          .value}/tracks?fields=items.track(name%2Cid%2Cartists.name)&limit=100&offset=$offset";
      final response = await http.get(Uri.parse(url), headers: {
        HttpHeaders.authorizationHeader: 'Bearer ${context
            .read(tokenProvider)
            .token}',
...
Text('Generator',textAlign: TextAlign.center,)
...
//   inactiveFgColor: Colors.white,
    ...
    initialValue: context.read(graphDataProvider).generator.keys.toList(),
...
IconButton(onPressed: (){
  ...
  downloadPlaylistData().then((value) => extractTrackMetrics(value).then((value) => buildPlaylist(value))
...
//          ElevatedButton(onPressed: null,child: Icon(Icons.delete)),
 ...
 //  Flexible(child:Container())
  ...
  double L1(List<double?> genValues,double musicValue, double musicDuration, double startTime ){
...
return result/window;
  ...
  double dist(Map<String,List<double?>> generator, Map<String,dynamic> music, double startTime) {
    int dims=0;
    double result=0;
    for (String metric in generator.keys){
      dims+=1;
      result+=L1(generator[metric]!,music[metric],music['duration_ms'],startTime);

    }
    return result/dims;
  }

  int bestMatch(Map<String,List<double?>> generator,List musics, double startTime){
  ...
  buildPlaylist(List value) {
    ...
    Map<String,List<double?>> generator = context.read(graphDataProvider).generator;
    int lastActivated = context.read(graphDataProvider).lastActivated;

    int tmp;
    while(playlistDuration<lastActivated*deltaTime.inMilliseconds && value.length>0 ){
 ...
 Future<List> extractTrackMetrics(List value) async {
   List trackMetrics=[];
    for (int i=0;i<value.length;i+=100){
      ...
      var url = "https://api.spotify.com/v1/audio-features?ids=" + buildIdsRequest(value,i,min(i+100,value.length));
