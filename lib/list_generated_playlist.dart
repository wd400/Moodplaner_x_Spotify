...
class ListGeneratedPlaylist extends StatelessWidget {
  ...
  const ListGeneratedPlaylist({Key? key, required this.items}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    const title = 'Long List';
    print(items);
    return Column(children:[Text('Playlist generated'), Flexible(child:ListView.builder(
...
Future<String?> buildNewPlaylist(String token,String userId, String name) async {
...
Future<bool> saveInPlaylist(String token, String playlistId,List playlist) async{
...
body: json.encode({"uris": tracksToList(playlist,i,min(playlist.length,i+100)),
        "position": i})
